...
- **Distance and time**: straight-line distance filters the radius. Travel time is estimated per station from that distance at a fixed average speed in v1 — enough to decide which time-of-use window a station falls into — behind a seam that a small routing call (Routes API Essentials) replaces once a provider key exists, at which point it supplies travel time only when the detail screen is opened or when results are sorted by arrival time, cached per station.
